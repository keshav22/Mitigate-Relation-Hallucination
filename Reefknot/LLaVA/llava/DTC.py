...
def DTC_function():
    ...
    def greedy_search_redefine(
        ...
        **model_kwargs,  
        ...
        dtc_layer_lambda = getattr(self.generation_config, "dtc_layer", None)
        ...
        self.dtc_first_yes_prob = None
        self.dtc_first_no_prob = None
        self.dtc_first_entropy = None
        # -----------------------------------
        ...
        while True:
            ...
            if self.dtc_first_yes_prob is None:
                self.dtc_first_yes_prob = yes_prob
                self.dtc_first_no_prob = no_prob
            ...
            if self.dtc_first_entropy is None:
                self.dtc_first_entropy = yes_no_entropy

            use_dtc = (threshold is not None) and (apha is not None) and (yes_no_entropy >= float(threshold))
            if use_dtc:
                relative_top = 0.1

                print("[DTC] Using DTC (entropy >= threshold)")
                ...
                if relative_top > 0.0:
                    ...
                    print(f"[DTC] probs_thresh {probs_thresh}")

                    mask = final_logits_norm < probs_thresh  # [bsz, vocab]

                    print("[DTC] Mask stats:", torch.max(mask), torch.min(mask))
                ...
                next_token_logits = process_logits.to(final_logits_step.dtype)
            else:
