...
def contains_relation(text: str) -> bool:
    """
    Returns True if text contains at least one verb (relation)
    """
    if not text:
        return False
    ...
    return any(tag in VERB_TAGS for _, tag in pos_tags)
    ...
    for line in tqdm(questions):
        ...
        if args.cd_mode == "patched_cd":
            ...
            if img_id not in gdino_boxes or len(gdino_boxes[img_id]) == 0:
                ...
                # --- helper: phrase -> is_relation? ---
                def is_relation_det(det):
                    return contains_relation(det.get("matched_phrase", ""))

                relation_dets = [det for det in detections if is_relation_det(det)]
                non_relation_dets = [det for det in detections if not is_relation_det(det)]

                # Decide which detections to noise (after deleting relations)
                if len(relation_dets) == 0:
                    print(f"[{img_id}] After removing relations, nothing left.")
                    if args.noise_target_mode == "single":
                        # fallback: noise highest-score overall
                        targets = [max(detections, key=lambda d: d.get("score", 0.0))]
                        print(f"[{img_id}] Fallback(single): noising highest-score overall.")
                    else:
                        # fallback: noise all overall detections
                        targets = detections
                        print(f"[{img_id}] Fallback(multi): noising ALL detections overall.")
                else:
                    if args.noise_target_mode == "single":
                        targets = [max(non_relation_dets, key=lambda d: d.get("score", 0.0))]
                    else:
                        targets = non_relation_dets
                ...
                if args.noise_target_mode == "single":
                    chosen = targets[0]
                    print(
                        f"[{img_id}] mode=single "
                        f"relations_removed={len(relation_dets)} "
                        f"non_relation_candidates={len(non_relation_dets)} "
                        f"chosen='{chosen.get('matched_phrase','')}' score={chosen.get('score',0.0):.4f} "
                        f"relation dets={relation_dets}"
                        f"non relation dets={non_relation_dets}"
                    )
                else:
                    # summarize for multi
                    top = max(targets, key=lambda d: d.get("score", 0.0)) if targets else None
                    print(
                        f"[{img_id}] mode=multi "
                        f"relations_removed={len(relation_dets)} "
                        f"noised_count={len(targets)} "
                        f"top_noised='{top.get('matched_phrase','') if top else ''}' "
                        f"relation dets = {relation_dets}"
                        f"non relation dets={non_relation_dets}"
                    )

                # ---- Debug image: draw ALL noised bboxes ----
